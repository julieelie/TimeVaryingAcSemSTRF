...
Job_Storage_location = '/tmp/LocalClusterJobStorage_Jelie';
NbTrialStim=10;
...
PaceParallelToolBox_r2014b('cores',ParamModel.BootstrapSTRF,'job_storage',Job_Storage_location)
...
delete(gcp('nocreate'))
if ~isempty(strfind(getenv('HOSTNAME'),'.savio')) || ~isempty(strfind(getenv('HOSTNAME'),'.brc')) % Check that we are on the savio cluster
