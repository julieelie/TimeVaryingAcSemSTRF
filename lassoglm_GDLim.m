function [B,stats] = lassoglm_GDLim(x,y,distr,varargin)
...
%                      TRUE, do scale X.
%     'RelTol'         Convergence thresholds for coordinate descent algorithm.
%                      The coordinate descent iterations will terminate
%                      when the relative change in the size of the
%                      estimated coefficients B drops below the threshold RelTol.coeff
%                      (Default: 1e-4. Legal range is (0,1)) or when the sum of the
%                       squared errors drops below RelTol.error (Default
%                       calculated as
%                       length(y)*(log(max(y)+0.26)-log(max(y)+0.25))^2
%                       which is right for a Poisson distribution only
...
% Note That there exists an internal switch button (FIGSwitch) l 1394 to see figures
% of the coefficients values inside the coordinate descent algorithm
tic
if nargin < 2
...
LRdefault = 1e-4;
reltoldefault.coeff=1e-4;
reltoldefault.error=length(y)*(log(max(y)+0.26)-log(max(y)+0.25))^2;
pnames = { 'alpha' 'numlambda' 'lambdaratio' 'lambda' ...
    'dfmax' 'standardize' 'reltol' 'cv' 'mcreps' ...
    'predictornames' 'options' };
dflts  = {  1       100       LRdefault     []      ...
     []      true          reltoldefault    'resubstitution'  1 ...
     {}               []};
...
% The following function calculate eta for the y input. In the case of a
% poisson distriution with log as link function eta=log(y+0.25). 0.25 makes
% sure log(y) does not = -Inf when y=0, mu=y+0.25
...
%dfmax is #non-zero coefficients. Then if X has any constant columns, we want to exclude them from the
% coordinate descent calculations.  The corresponding coefficients
% will be returned as zero, ever_active contains the non-constant parameter columns
...
[B,Intercept,lambda,deviance] = ...
    ...
    penalizedFit,mu,eta,dataClass,userSuppliedLambda,nullDev,nullIntercept);  %mu=y+0.25, eta=log(y+0.25), nullDev= deviance of the null model (model fit with x=ones), nullIntercept=coefficients of the null model
...
% === 'RelTol' parameter ===
%
if ~isscalar(reltol.coeff) || ~isreal(reltol.coeff) || ~isfinite(reltol.coeff) || reltol.coeff <= 0 || reltol.coeff >= 1
    error(message('stats:lassoGlm:InvalidRelTolCoeff'));
end

if ~isscalar(reltol.error) || ~isreal(reltol.error) || ~isfinite(reltol.error) || reltol.error <= 0
    error(message('stats:lassoGlm:InvalidRelTolError'));
end
...
convcrit = max(1e-6,2*reltol.coeff);
...
while iter <= iterLim
    ...
    deta = dlinkFun(mu);
    z = eta + (y - mu) .* deta;% This line corresponds to equation 4.24 in An Introduction to generalized linear models from Annette J. Dobson (2002)

    % Compute IRLS weights the inverse of the variance function
    sqrtw = sqrt(pwts) ./ (abs(deta) .* sqrtvarFun(mu));% This corresponds to the square root of the equation of the weights 4.23 page 64 in An Introduction to generalized linear models from Annette J. Dobson (2002)
...
function [b,varargout] = ...
    ...
    FIGSwitch=0; %Switch this button to 1 if you want to see some figures about b values over iterations
    weights = weights(:)';
    ...
    % Iterative coordinate descent until converged or until max
    % #iterations is reached
    CD_iter = 0;
    CD_iterLim = 1000;
    if FIGSwitch
        NumActive = nan(CD_iterLim,1);
        Btrack = nan(length(b),CD_iterLim);
        Error = nan(CD_iterLim,1);
    end
    while CD_iter <= CD_iterLim % this is set to true in the original version of lassoglm
        CD_iter = CD_iter + 1;
        bold = b;
        old_active = active;

        [b,active] = cdescentCycleNoRecalc(X,wX,wX2,Y, b,active,shrinkFactor,threshold);
        Error_local=sum((Y - X(:,active)*b(active,:)).^2);
        

        if Error_local<reltol.error % the error is below the best we can predict given the max spike count, we're done (added by JEE)
            if FIGSwitch
                NumActive(CD_iter) = sum(active);
                Btrack(:,CD_iter)=b;
                Error(CD_iter)=Error_local;
            end
            break
            
        elseif ~any( abs(b(old_active) - bold(old_active)) > reltol.coeff * max(1.0,abs(bold(old_active))) ) % This is the original Matlab line
        % testing a threshold for each parameter and only breaking if none
        % of the parameters changed sufficiently
        %elseif norm(b(old_active) - bold(old_active)) < reltol.coeff * max(1.0,norm(bold(old_active))) % This new term test for the norm of the vector (more averaged measurements)
            % Cycling over the active set converged.
            % Error is still higher that the best we could potentially do
            % given the max spike count (reltol.error)
            ...
            if FIGSwitch
                NumActive(CD_iter) = sum(active);
                Btrack(:,CD_iter)=b;
                Error(CD_iter)=sum((Y - X(:,active)*b(active,:)).^2);
            end
            ...
            if isequal(new_active, active)
                ...
                if ~any( abs(b(super_active) - bold(super_active)) > reltol.coeff * max(1.0,abs(bold(super_active))) ) %This is the original Matlab line
        %testing a threshold for each parameter and only breaking if none
        %of the parameters changed sufficiently
                %if norm(b(super_active) - bold(super_active)) < reltol.coeff * max(1.0,norm(bold(super_active))) % This new term test for the norm of the vector (more averaged measurements)
        ...
        if CD_iter > CD_iterLim
            fprintf('stats:lassoGlm:Penalizedwls:CDIterationLimit\n');
        end
    ...
    if FIGSwitch
        figure(201)
        subplot(1,4,1)
        plot(NumActive(1:CD_iter),'.-');
        xlabel('Iterations coordinate descent')
        ylabel('size of the set of active parameters')
        subplot(1,4,2)
        plot(NumActive(1:CD_iter)*100/length(b),'.-');
        xlabel('Iterations coordinate descent')
        ylabel('percentage of active parameters')
        subplot(1,4,3)
        plot(Btrack(:,1:CD_iter)','.-');
        xlabel('Iterations coordinate descent')
        ylabel('values of coefficients b')
        subplot(1,4,4)
        plot(Error(1:CD_iter),'.-');
        xlabel('Iterations coordinate descent')
