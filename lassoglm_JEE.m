function [B,stats] = lassoglm_JEE(x,y,distr,varargin)
...
%   JEE modified penalizedWls() for Theunissen Lab. to eliminate the
%   infinite loop if the gradient descent is taking longer than 2 min, when
%   it supposed to take less than 10 sec. 2015.06.15
...
% generate a sequence.
if isempty(lambda)
    fprintf('generate sequence of lambda\n')
...
fprintf('Convert lambda penalty to match total log-likelihood\n')
...
% ------------------------------------------
fprintf('Order results by ascending lambda\n')
...
while iter <= iterLim
    iter = iter+1;
fprintf('Iteration glmIRS %d max %d\n',iter,iterLim)
    ...
    fprintf('In glmIRS: l919: Compute predicted mean using inverse link function\n')
    mu = ilinkFun(eta);
...
[B,mu,eta,active] = glmIRLS(X,Y,distr,offset,pwts,dataClass,N, ...
    sqrtvarFun,linkFun,dlinkFun,ilinkFun,b,active,mu,muLims, ...
    wlsfit,nullDev,devFun,reltol);
fprintf('Back to glmIRLSwrapper\n')
...
%
fprintf('weights and standardization\n')
...
for i = 1:nLambda
    ...
    if sum(active) > dfmax
        ...
        B = B(:,1:(i-1));
        Extras = Extras(:,1:(i-1));
        fprintf('In LassoGLM: breaking lamba loop1\n')
        break
    end
    ...
    if ~(userSuppliedLambda || isempty(nullDev))
        if extras.Deviance < 1.0e-3 * nullDev
            ...
            B = B(:,1:i);
            Extras = Extras(:,1:i);
            fprintf('In LassoGLM: breaking lamba loop2\n')
            break
        end
    end
...
fprintf('penalizedWls\n')
    ...
    % Iterative coordinate descent until converged or max iteration is
    % reached
    TrueLoop_iter=0;
    TrueLoop_iterLim=1000;
    tic
    while true
        TrueLoop_iter=TrueLoop_iter+1;
        bold = b;
        old_active = active;
        ...
        if ~any( abs(b(old_active) - bold(old_active)) > reltol * max(1.0,abs(bold(old_active))) )
            ...
            if isequal(new_active, active)
                fprintf('Exit true loop because active parameters are the same after %d iterations\n',TrueLoop_iter)
                toc
                break
            else
                ...
                if ~any( abs(b(super_active) - bold(super_active)) > reltol * max(1.0,abs(bold(super_active))) )
                    ...
                    fprintf('Exit true loop because active parameters did not change much after %d iterations\n',TrueLoop_iter)
                    toc
                    break
                else
                    active = new_active;
        ...
        if TrueLoop_iter>TrueLoop_iterLim
            fprintf('Reached max %d iterations for gradient descent for that lambda',TrueLoop_iter)
            toc
            break
        end
    end
...
fprintf('glmPenalizedWlsWrapper\n')
