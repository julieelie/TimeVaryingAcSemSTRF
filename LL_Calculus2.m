function LL=LL_Calculus2(y,mu,Scale,SUM)
if nargin<2
    mu=y;
end
...
%     Scale=20;%Best value for our data with Win = 10ms according to graph that can be obtained in the commented section below
%     muLims = 1/Scale;
else
...
if  any((y==0).*(mu==0))
    mu2 = mu+(y==0).*(mu==0); %ylog(y)=0 when y=0 so force mu=1 for these particular examples
else
    mu2 = mu;
end

if any(mu2 < muLims(1))
    mu2 = max(mu2,muLims(1)); %threshold mu so the log calculation does not blow up when mu=0
end

LL= y.*log(mu2) -log(factorial(y)) - mu;
...
% R=0:1:6;% Hypothetical spike count for y
% SCALE=2:2:20;%Scales
...
%     hold on
%     for ss=1:length(SCALE)
%         LL=LL_Calculus2(Res,M,SCALE(ss),0);
%         plot(M,LL,'.-')
%         pause(1)
%     end
