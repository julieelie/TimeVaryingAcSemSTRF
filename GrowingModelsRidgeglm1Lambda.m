function [Deviance, NeuroRes, VOC, Wins] = GrowingModelsRidgeglm1Lambda(Spectro, VocType, PSTH, Trials, Emitter, MinWin, MaxWin, Increment, ResDelay, NeuroRes, DISTR, LINK, Cellname)
...
NbTrialStim=10;
LAMBDARATIO=1e-4;
OFFSET=1;%set 0 if you don't want the Acsem model to be calculated with an offset obtained from the semantic model
% set to 1 for AcSem calculated with Semantic Offset
%set to 2 for AcSem calculated with Semantic and Acoustic Offset
ModelChoice=[1 1 0 0 0];% This logic vector indicates which models should
...
LAMBDA=[];%set to [] if you want to explore lamba values %53.3
LINK_AR='identity';
...
Alphas=[0.001 1]; % STRFs are easier to interpret using ridge than using Lasso and deviances are similar.
...
if ModelChoice(4) && ModelChoice(1)
...
elseif ModelChoice(5) && ~ModelChoice(2)
...
%     
    ...
    LL_AR2=nan(BootstrapSTRF,length(Alphas));
...
%             X_voc_Train_local=2*SemBoost.*X_voc_Train;
            ...
            %MDL_Sem.Coefficients.Estimate=glmfit(X_voc_Train,y_Train,DISTR,'link',LINK);
            ...
            if isempty(LAMBDA)
                ...
                [B_Ac, FitInfo_Ac]=lassoglm(x_Train,y_Train,DISTR,'Alpha',Alpha,'Link',LINK,'Standardize',0,'Lambda',LAMBDA);
            end
            ...
            elseif OFFSET && ~isempty(LAMBDA)
                [B_AcSem_local, FitInfo_AcSem_local]=lassoglm([SemBoost.*X_voc_Train x_Train],y_Train,DISTR,'Alpha',Alpha,'Link',LINK,'Standardize',0,'Lambda',LAMBDA, 'Offset', Sem_Offset);
            ...
            elseif ~OFFSET && ~isempty(LAMBDA)
                [B_AcSem_local, FitInfo_AcSem]=lassoglm([SemBoost.*X_voc_Train x_Train],y_Train,DISTR,'Alpha',Alpha,'Link','Standardize',0,'Lambda',LAMBDA);
                B_AcSem_local(1:length(MDL_Sem.Coefficients.Estimate(2:end)))=B_AcSem_local(1:length(MDL_Sem.Coefficients.Estimate(2:end))).*SemBoost;
                B_AcSem=B_AcSem_local;
            end
            ...
            if FitPerTrial
                ...
                if isempty(y_dev_old)
                    ...
                    MDL_AR=fitglm(y_input_AR,y_Train,'Distribution',DISTR,'Link',LINK_AR,'intercept',false);
                    %[MDL_AR.Coefficients.Estimate,MDL_AR.Deviance,MDL_AR.stats]=glmfit(y_input_AR,y_Train,DISTR,'link',LINK_AR,'constant','off');
                end
            ...
            if FitPerTrial
                ...
                LL_BestGuess(bb,aa) = LL_Calculus(y_Val,y_Val_bestGuess);
                LL_WorseGuess(bb,aa) = LL_Calculus(y_Val,y_Val_worseGuess);
                ...
                for ll=1:NbL_Ac
                    ...
                    if strcmp(DISTR,'poisson')
                         ...
                         Deviance_M_Ac(ll) = -2*(LL_Ac(ll)-LL_BestGuess(bb,aa));
            ...
            if FitPerTrial
                ...
                for ll=1:NbL_AcSem
                    ...
                    if strcmp(DISTR,'poisson')
                        ...
                        Deviance_M_AcSem(ll) = -2*(LL_AcSem(ll)-LL_BestGuess(bb,aa));
                    elseif strcmp(DISTR,'normal')
            ...
            if strcmp(DISTR,'poisson')
                LL_BestModel_Sem(bb,aa) = LL_Calculus(y_Val, ypred_Sem);
                Deviance_BestModel_Sem(bb,aa) = -2*(LL_BestModel_Sem(bb,aa)-LL_BestGuess(bb,aa));
            ...
            if ~isempty(y_dev_old)
                ...
                ypred_AR = glmval(MDL_AR.Coefficients.Estimate,y_Val_input_AR,LINK_AR, 'constant','off');
            end
            LL_AR(bb,aa) = LL_Calculus(y_Val,ypred_AR);
            if mm>1
                MDL_AR2=fitglm(y_Val_input_AR,y_Val,'Distribution',DISTR,'Link',LINK_AR,'intercept',false);
                %[MDL_AR2.Coefficients.Estimate,MDL_AR2.Deviance,MDL_AR2.stats]=glmfit(y_Val_input_AR,y_Val,DISTR,'link',LINK_AR,'constant','off');
                LL_AR2(bb,aa)=LL_Calculus(y_Val,MDL_AR2.Fitted.Response);
            else
                LL_AR2(bb,aa)=LL_AR(bb,aa); % Set the value to BestGuess model value since LL_AR=LL_BestGuess when mm=1
            end
            if FIG>1 && mm>1
                figure()
                plot(y_Val,y_Val_bestGuess,'r.','MarkerSize',10)
                hold on
                plot(y_Val,ypred_AR,'b.','MarkerSize',10)
                ...
                title(sprintf('LL Ceiling Model=%f\nLL Auto-Regressive Model train fit=%f\nLL Auto-Regressive Model Val fit=%f',LL_BestGuess(bb),LL_AR(bb),LL_AR2(bb)))
           ...
           %% Store the smallest Deviance Value, the corresponding best lambda and the corresponding best model
            ...
            FitIndexAR_Acoustic_local=(LL_BestModel_Acoustic(bb,aa)-LL_WorseGuess(bb,aa))/(LL_AR(bb,aa)-LL_WorseGuess(bb,aa));
            FitIndexARVal_Acoustic_local=(LL_BestModel_Acoustic(bb,aa)-LL_WorseGuess(bb,aa))/(LL_AR2(bb,aa)-LL_WorseGuess(bb,aa));
            ...
            FitIndexAR_AcSem_local=(LL_BestModel_AcSem(bb,aa)-LL_WorseGuess(bb,aa))/(LL_AR(bb,aa)-LL_WorseGuess(bb,aa));
            FitIndexARVal_AcSem_local=(LL_BestModel_AcSem(bb,aa)-LL_WorseGuess(bb,aa))/(LL_AR2(bb,aa)-LL_WorseGuess(bb,aa));
            ...
            FitIndexAR_Sem_local=(LL_BestModel_Sem(bb,aa)-LL_WorseGuess(bb,aa))/(LL_AR(bb,aa)-LL_WorseGuess(bb,aa));
            FitIndexARVal_Sem_local=(LL_BestModel_Sem(bb,aa)-LL_WorseGuess(bb,aa))/(LL_AR2(bb,aa)-LL_WorseGuess(bb,aa));
            ...
            if OFFSET>1
                ...
                if isempty(LAMBDA)
                    ...
                    [B_AcSem_local2, FitInfo_AcSem_local2]=lassoglm([SemBoost.*X_voc_Train x_Train],y_Train,DISTR,'Alpha',Alpha,'Link',LINK,'Standardize',0,'Lambda',LAMBDA, 'Offset', Ac_Offset);
                end
                ...
                if FitPerTrial
                    ...
                    for ll=1:NbL_AcSem2
                        ...
                        if strcmp(DISTR,'poisson')
                            LL_AcSem2(ll) = LL_Calculus(y_Val,ypred_AcSem2(1:yy,ll));
                            Deviance_M_AcSem2(ll) = -2*(LL_AcSem2(ll)-LL_BestGuess(bb,aa));
                        elseif strcmp(DISTR,'normal')
                ...
                FitIndexAR_AcSem2_local=(LL_BestModel_AcSem2(bb,aa)-LL_WorseGuess(bb,aa))/(LL_AR(bb,aa)-LL_WorseGuess(bb,aa));
                FitIndexARVal_AcSem2_local=(LL_BestModel_AcSem2(bb,aa)-LL_WorseGuess(bb,aa))/(LL_AR2(bb,aa)-LL_WorseGuess(bb,aa));
            ...
            if FIG>1
                ...
                for ll=1:NbL_Ac
                    if ll==Dev_min_index_Ac
                        ...
                        axis xy
                        title(sprintf('THIS IS THE ONE!!!\nSTRF Ac log of lambda=%f\n Deviance=%f FitIndex=%f\nFitIndexAR=%f',log10(FitInfo_Ac.Lambda(ll)),Deviance_BestModel_Acoustic(bb,aa),FitIndex_Acoustic_local(bb,aa),FitIndexAR_Acoustic_local))
                    else
                        figure(5)
                        if ZC==1
                            Lasso_STRF=reshape(B_Ac(:,ll)'.*x_Train_std,Df,Dt);
                ...
                for ll=1:NbL_AcSem
                    if ll==Dev_min_index_AcSem
                        ...
                        axis xy
                        title(sprintf('THIS IS THE ONE!!!\nSTRF AcSem log of lambda=%f\n Deviance=%f FitIndex=%f\nFitIndexAR=%f',log10(FitInfo_AcSem.Lambda(ll)),Deviance_BestModel_AcSem(bb,aa),FitIndex_AcSem_local(bb,aa),FitIndexAR_AcSem_local))
                        ss=subplot(1,2,2);
                ...
                if OFFSET>1
                    ...
                    for ll=1:NbL_AcSem2
                        if ll==Dev_min_index_AcSem2
                            ...
                            axis xy
                            title(sprintf('THIS IS THE ONE!!!\nSTRF AcSem2 log of lambda=%f\n Deviance=%f FitIndex=%f\nFitIndexAR=%f',log10(FitInfo_AcSem2.Lambda(ll)),Deviance_BestModel_AcSem2(bb,aa),FitIndex_AcSem2_local(bb,aa),FitIndexAR_AcSem2_local))
                            ss=subplot(1,2,2);
                            % Make sure we have one stim per category in the
                            % training set or set the coefficient value of the
                            % model for that category as the average of
                ...
                title(sprintf('Coefficients Categories Sem Model Deviance=%f\nFitIndex=%f\nFitIndexAR=%f',Deviance_BestModel_Sem(bb,aa),FitIndex_Sem_local(bb,aa),FitIndexAR_Sem_local));
                ...
                if OFFSET>1
                    Legend{3}='Acoustic + Semantic SemOFFSET';
                    Legend{4}='Acoustic + Semantic AcOFFSET';
                ...
                for jj=1:length(LocalModelPredict)
                    ...
                    if strcmp(NeuroRes,'mean') && ~FitPerTrial
                        xlabel('observed spike rate (spike per ms)')
                        ylabel('predicted spike rate (spike per ms)')
                    elseif FitPerTrial
                        xlabel(sprintf('spike count (%d ms)',Win))
                        ylabel(sprintf('spike count (%d ms)',Win))
                    end
        ...
        if OFFSET>1
            ...
            FitIndexARVal_AcSem2(aa)=(sum(LL_BestModel_AcSem2(:,aa))-sum(LL_WorseGuess(:,aa)))/(sum(LL_AR2(:,aa))-sum(LL_WorseGuess(:,aa)));
        end
        ...
        if FIG>0
            ...
            %Colors='brmgcky';
            figure(13)
            plot(1:BootstrapSTRF,LL_BestModel_Acoustic,'b.-',1:BootstrapSTRF,LL_BestModel_AcSem,'m.-',1:BootstrapSTRF,LL_BestModel_AcSem2,'r.-',1:BootstrapSTRF,LL_BestGuess,'y.-',1:BootstrapSTRF,LL_WorseGuess,'k.-',1:BootstrapSTRF,LL_AR,'c.-',1:BootstrapSTRF,LL_BestModel_Sem,'g.-',1:BootstrapSTRF,LL_AR2,'c.--');
            hold on
            legend(models);
            ylabel('LogLikelihood');
            xlabel('Bootstrap');
            hline(mean(LL_BestModel_Acoustic),'b:')
            hline(mean(LL_BestModel_AcSem2),'r:')
            hline(mean(LL_BestModel_AcSem),'m:')
            hline(mean(LL_BestModel_Sem),'g:')
            hline(mean(LL_WorseGuess),'k:')
            hline(mean(LL_BestGuess),'y:')
            hline(mean(LL_AR),'c:')
            hline(mean(LL_AR2),'c:o')
            hold off
        end
    ...
    if OFFSET>1
        ...
        Deviance.AcSem2.FitIndexARVal{mm}=FitIndexARVal_AcSem2;
